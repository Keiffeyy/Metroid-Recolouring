 ...
 * 
 * @author (Busch2207 (Moritz L.)) edited by David Liu and Jerry Xu 
 * @version (09.11.2013)
 */
public abstract class Collision extends Actor
{
    ...
    /**
     * Checks if the object is one a platform
     * 
     * @author Jerry Xu 
     * @return boolean On or Off platform
     */
    protected boolean onPlatform()
    {
        Actor under = getOneObjectAtOffset(0, 20, Tile.class);
        if (under != null)
        {
            return true;
        }

        Actor under1 = getOneObjectAtOffset(0, 10, Tile.class);
        if (under != null)
        {
            return true;
        }
    ...
    /**
     * see if its touching the side of a tile
     * @author David Liu
     * @return boolean
     * @param left Left or Right
     * @param a The object to detect from (just put "this")
     */
    protected boolean touchSide(boolean left,Actor a){
        int thisX = a.getX();
        int thisY = a.getY();
        int imgW = a.getImage().getWidth()/2;
         int imgH = a.getImage().getHeight()/2;
        
        List<Tile> list = getObjectsInRange(imgW*2, Tile.class);
        for(Tile t : list){
            int tileX = t.getX();
            int tileY = t.getY();
            if(Math.abs(tileY-thisY)<imgH){
                if(left){
                    if(((thisX-imgW)-(tileX+24))<24&&((thisX-imgW)-(tileX+24))>-24){
                        return true;
                    }
                }else{
                    if(((tileX-24)-(thisX+imgW))<24&&((tileX-24)-(thisX+imgW))>-24){
                        return true;
                    }
                }
            }
